# SiteChat Agent CMS - Blazor Web Application

A modern Blazor-based Content Management System with an integrated AI Chat Agent.

## Overview

This is a complete, runnable Blazor CMS project that integrates with the SiteChat Agent. The CMS provides a full-featured web interface with an embedded chat agent powered by Azure OpenAI.

## Project Structure

```
src/cms/
├── SiteChatCMS.sln          # Visual Studio solution file
├── Server/                    # Server-side Blazor project
│   ├── SiteChatCMS.csproj   # Project file
...
1. **Open in Visual Studio**:
   - Double-click `SiteChatCMS.sln` to open the solution in Visual Studio
   - Or from Visual Studio: File → Open → Project/Solution → Select `SiteChatCMS.sln`
...
1. **Start the Backend Chat Service** (in terminal 1):
   ```bash
   # From the repository root
   cd /home/runner/work/sitechat-agent/sitechat-agent
   python main.py
   ```
...
```bash
cd src/cms
dotnet publish Server/SiteChatCMS.csproj -c Release -o ../../publish
```
...
- Repository Issues: https://github.com/orkinosai25-org/sitechat-agent/issues
...
Enjoy using SiteChat Agent CMS! 🚀
